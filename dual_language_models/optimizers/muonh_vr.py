        ...
        for group in self.param_groups:
            if group["use_muon"]:
                for p in group["params"]:
                    ...
                    print(f"Update norm for parameter of shape {p.shape}: {update.norm():.4f}")
                    update.mul_(state["R"] / max(update.norm(), 1e-10))
                    print(f"New update norm for parameter of shape {p.shape}: {update.norm():.4f}")
                    print(f"R before update for parameter of shape {p.shape}: {state['R'].item():.4f}")

                    # if group["weight_decay"] and had_grad:
                    #     p.mul_(1 - group["lr"] * group["weight_decay"])
                    p.add_(update, alpha=-group["lr"])
                    print(f"Parameter norm after update for parameter of shape {p.shape}: {p.norm():.4f}")
                    p.mul_(state["R"] / p.norm())  # Renormalize to original norm
            else:
                for p in group["params"]:
                    had_grad = p.grad is not None
