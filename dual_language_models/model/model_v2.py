...
class Model(nn.Module):
    ...
    def __init__(self, config: dict) -> None:
        ...
        # Loss is kept as an attribute but should be called OUTSIDE torch.compile
        # to avoid graph breaks in the encoder/classifier hot path.
        if config.use_liger:
...
class FeedForward(nn.Module):
    ...
    def __init__(self, config: dict) -> None:
        ...
        self.up_proj: MultiCastedLinearOrtho
    ...
    def forward(self, hidden_layer: torch.Tensor) -> torch.Tensor:
        ...
        hidden_layer = self.pre_norm(hidden_layer.float()).type_as(hidden_layer)
        output = self.up_proj(hidden_layer)

        x, gate = output.chunk(2, dim=-1)
        if self.use_liger:
            output = LigerSiLUMulFunction.apply(gate, x)
        else:
            output = x * F.silu(gate)
...
class RotaryPositionalEmbeddings(nn.Module):
    ...
    def __init__(self, config: dict) -> None:
        ...
        if self.use_liger:
            # Liger RoPE expects cos/sin shape: (1, seq_len, head_dim)
            # It uses only the first half internally, but we store full head_dim for consistency
            embedding_full = torch.cat([embedding, embedding], dim=-1).unsqueeze(0)  # (1, seq_len, head_dim)
            self.register_buffer("cos_matrix", embedding_full.cos(), persistent=False)
            self.register_buffer("sin_matrix", embedding_full.sin(), persistent=False)
        else:
            # Native RoPE expects cos/sin shape: (1, seq_len, head_dim) — unsqueeze for batch broadcasting
            embedding_full = torch.cat([embedding, embedding], dim=-1).unsqueeze(0)
            self.register_buffer("cos_matrix", embedding_full.cos(), persistent=False)
            self.register_buffer("sin_matrix", embedding_full.sin(), persistent=False)
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.use_liger:
            # liger_rotary_pos_emb expects:
            #   q: (bsz, n_q_head, seq_len, head_dim)
            #   k: (bsz, n_kv_head, seq_len, head_dim)
            #   cos: (1, seq_len, head_dim)
            #   sin: (1, seq_len, head_dim)
            q_layer, k_layer = liger_rotary_pos_emb(
                q_layer, k_layer,
                self.cos_matrix[:, :seq_len, :],
                self.sin_matrix[:, :seq_len, :],
            )
        else:
