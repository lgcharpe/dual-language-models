...
class MultiCastedLinearOrtho(nn.Module):
    ...
    def _current_versions(self) -> tuple[int, ...]:
        return tuple(weight._version for weight in self.weights)

    def fused_weight(self) -> torch.Tensor:
        versions = self._current_versions()
        if (not self._cache_valid) or (self._cached_weight is None) or (versions != self._cached_versions):
            self._cached_weight = torch.cat([weight for weight in self.weights], dim=0)
            self._cached_versions = versions
            self._cache_valid = True
        return self._cached_weight

    def forward(self, x):
        return F.linear(x, self.fused_weight().type_as(x), bias=self.bias.type_as(x) if self.bias is not None else None)
...
class Model(nn.Module):
    ...
    def get_encodings(
        ...
        word_embeddings = self.embedding(input_ids)
        encodings = self.encoder(word_embeddings, doc_ids, attention_mask=attention_mask)
    ...
    def forward(
        ...
        encodings = self.encoder(word_embeddings, doc_ids, attention_mask=attention_mask).bfloat16()
...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            hidden_layer = layer(hidden_layer, doc_ids, attention_mask=attention_mask)
...
class Layer(nn.Module):
    ...
    def forward(
        ...
        attention_layer = self.attention(hidden_layer, doc_ids, attention_mask=attention_mask)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is None:
            ...
            mixed_mask = attention_mask

            def score_mod(score, b, h, q_idx, kv_idx):
                return torch.where(mixed_mask[b, q_idx, kv_idx], score, -float("inf"))

        block_mask = self.mask
        if query_length != self.sequence_length or key_length != self.sequence_length:
            block_mask = self.mask._adjust(query_length, key_length)

        output = flex_attention(query, key, value, block_mask=block_mask, score_mod=score_mod)
