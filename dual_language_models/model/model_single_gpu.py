...
class Model(nn.Module):
    ...
    def create_mask(self, device: torch.device, batch_size: int) -> None:
        self.encoder._create_mask(device, batch_size)
    ...
    def forward(self, input_ids: torch.Tensor, doc_ids: torch.Tensor, causal_mask: torch.Tensor, labels: torch.Tensor | None = None) -> ModelOutput:
        ...
        if labels is not None:
            ...
            with torch.no_grad():
                output.accuracy = (logits.argmax(-1) == gold_labels).float().detach()
...
class Classifier(nn.Module):
    ...
    def __init__(self, config: Namespace, embedding_weights: nn.Parameter) -> None:
        super().__init__()
